...
export function PlannerPage({ data, customerById, onMove, onOpenSettings }: { data: ErpData; customerById: (id: string) => Customer | undefined; onMove: (vehicleId: string, date: string) => void; onOpenSettings: () => void }) {
  ...
  return <>
    ...
    <section className="panel table-panel"><div className="panel-head"><div><span className="eyebrow">CONSEGNE E BLOCCHI</span><h3>Vetture pianificate</h3></div></div><div className="table-wrap"><table><thead><tr><th>Vettura</th><th>Ore</th><th>Consegna richiesta</th><th>Consegna calcolata</th><th>Responso</th><th>Segnalazioni</th><th>Sposta lavorazione</th></tr></thead><tbody>{data.vehicles.filter((vehicle) => vehicle.status !== 'Consegnata' && vehicle.estimatedHours > vehicle.workedHours).map((vehicle) => { const plan = vehicleResult(vehicle.id); const openReports = (data.production?.reports ?? []).filter((report) => report.vehicleId === vehicle.id && !report.resolvedAt).length; return <tr className={plan?.blocked ? 'blocked-row' : ''} key={vehicle.id}><td><strong className="plate">{vehicle.plate}</strong><small>{plan?.blocked ? `Bloccata: ${vehicle.blockReason || 'ricambi mancanti'}` : `${vehicle.make} ${vehicle.model}`}</small></td><td>{Math.max(0, vehicle.estimatedHours - vehicle.workedHours)} h</td><td>{vehicle.requestedDeliveryDate || 'Non indicata'}</td><td>{plan?.calculatedDeliveryDate || 'Non calcolabile'}</td><td><TrafficLight status={plan?.status ?? 'neutral'} />{plan?.status === 'red' && <small>Mancano {plan.missingHours} h · ritardo {plan.delayWorkingDays} giorni · consigliata {plan.suggestedDate}</small>}</td><td>{openReports ? <span className="tag">{openReports}</span> : '—'}</td><td><input aria-label={`Sposta ${vehicle.plate}`} type="date" value={vehicle.manualPlanningDate} onChange={(event) => onMove(vehicle.id, event.target.value)} disabled={plan?.blocked} /></td></tr> })}</tbody></table></div></section>
  </>
}
